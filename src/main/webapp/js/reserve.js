...
checkValid.prototype = {
  ...
  emailValidTest: (emailValue, target) => {
    if (
      !(emailValue === '') &&
      /^[a-zA-Z0-9+-_.]+@[a-zA-Z0-9-]+\.[a-zA-Z0-9-.]+$/.test(emailValue)
    )
      return true
    else {
      if (target === 'email')
        $('#warning_msg_email').css('visibility', 'visible')
      return false
    }
  },
  ...
  checkingAllCondition: function (
    ...
    if (
      ...
      if (!emailValid) $('#warning_msg_email').css('visibility', 'visible')
        ...
        $('#warning_msg_name').css('visibility', 'visible')
...
function templatingProductPriceInfo(productPriceInfo) {
  const Section_ticketQuantity = document.querySelector(
    '#Section_ticketQuantity',
  ).innerHTML
  let resultHTML = ''
  ...
  if (0 != productPriceInfo.length) {
    productPriceInfo.forEach(function (productPriceInfo, index) {
      productPriceInfo.priceStringType = priceType.priceTypeToString(
        productPriceInfo.priceTypeName,
      )
...
countControlTap.prototype = {
  registerCalcuateFunction: function (qtyTab) {
    ...
    const minusBtn = qtyTab.querySelector(
      '.btn_plus_minus.spr_book2.ico_minus3',
    )
...
function addPriceInfo() {
  ...
  for (let i = 0; i < priceTabs.length; i++) {
    ...
    reservationPriceIdInput.setAttribute(
      'name',
      'reservationInfoPrices[' + i + ']' + '.productPriceId',
    )
    reservationPriceIdInput.setAttribute('value', priceId)

    reservationPriceCountInput.setAttribute('type', 'hidden')
    reservationPriceCountInput.setAttribute(
      'name',
      'reservationInfoPrices[' + i + ']' + '.count',
    )
    reservationPriceCountInput.setAttribute('value', countValue)

    document
      .querySelector('.form_horizontal')
      .appendChild(reservationPriceIdInput)
    document
      .querySelector('.form_horizontal')
      .appendChild(reservationPriceCountInput)
  }
}
