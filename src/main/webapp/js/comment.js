...
function countTextByteForGuide(textarea) {
	...
	textarea.addEventListener("keyup", evt => {
		...
		const nowTextLength = target.value.length;
...
function clickReviewSubmitBtn() {
	document.querySelector(".bk_btn").addEventListener("click", evt => {
		...
		const url = "/reservation/api/reservations/" + reservationInfoId + "/comment?" +
			"comment=" + comment + "&productId=" + productId + "&score=" + score;
