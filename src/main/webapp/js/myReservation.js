...
function templatingReservationInfo(reservationList) {
  ...
  const canceledCardItemTemplate = document.querySelector(
    '#canceledCardTemplate',
  ).innerHTML
  const usedCardItemTemplate = document.querySelector('#usedCardTemplate')
    .innerHTML
  ...
  if (reservationList.length === 0) {
    ...
    reservationList.forEach(function (info) {
      if (info.reservationInfo.cancelFlag == 0) {
        ...
        resultHTMLForUsed += templating.bindingTemplate(
          usedCardItemTemplate,
          info,
        )
      } else if (info.reservationInfo.cancelFlag != 0) {
        cancelItemCount++
        resultHTMLForCancel += templating.bindingTemplate(
          canceledCardItemTemplate,
          info,
        )
      }
    })
...
function registerHelperForhandlebars() {
  ...
  Handlebars.registerHelper('caculateTotalPrice', function (
    reservationInfoPrice,
  ) {
    let totalPrice = 0
    reservationInfoPrice.forEach(
      (index) => (totalPrice += index.price * index.count),
    )
...
function clickReservationCancelBtn() {
  ...
  confirmedList.addEventListener('click', function (evt) {
    if (evt.target.className == 'btn') {
      ...
      let confirmedList = document.querySelector('.card.confirmed')
      let clickedBookingNumber = parseInt(
        clickedReservationItem.querySelector('#bookingNumber').innerText,
      )
      let clickedItemTitle = clickedReservationItem.querySelector('.tit')
        .innerText
      let clickedItemPeriod = clickedReservationItem.querySelector('#period')
        .innerText
      ...
      clickCancelPopupBtn(
        ...
        clickedItemTitle,
        clickedItemPeriod,
        clickedBookingNumber,
      )
      clickClosePopUpBtn()
    } else if (evt.target.parentElement.className == 'btn') {
      let clickedReservationItem =
...
function clickCancelPopupBtn(title, period, reservationId) {
  ...
  cancelPopup
    .querySelector('.pop_bottom_btnarea')
    .addEventListener('click', (evt) => {
      if (
        (evt.target.parentElement.id ||
          evt.target.parentElement.parentElement.id) === 'cancel_pop_noBtn'
      ) {
        cancelPopup.style.display = 'none'
      } else if (
        (evt.target.parentElement.id ||
          evt.target.parentElement.parentElement.id) === 'cancel_pop_yesBtn'
      ) {
