window.addEventListener('load', function () {
  const displayInfoId = document
    .querySelector("meta[name='displayInfoId']")
...
function productDetailAjax(displayInfoId) {
  ...
  oReq.addEventListener('load', function () {
    ...
    const { product, display, image } = json

    const productImageList = document.querySelector('.visual_img')

    imageList.templatingProductImage(
      productImageList,
      productImageInfo,
      displayInfo,
      4,
...
let imageList = {
  filterImageInfoListByType: function (productImageInfo, typeString) {
    let typeFilteredInfoList = productImageInfo.filter(function (imageInfo) {
      return imageInfo.type == typeString
    })
    return typeFilteredInfoList
  },

  orderImageSequence: function (
    typeMainImageInfoList,
    typeEtcImageInfoList,
    LIMIT,
  ) {
    let orderedList = []
    let etcImagecount = 0
  ...
  templatingProductImage: function (
    productImageList,
    productImageInfo,
    displayInfo,
    LIMIT,
  ) {
    const productImageItemTemplate = document.querySelector('#productImageItem')
      .innerHTML
    let resultHTML = ''
    let typeMainImageInfoList = this.filterImageInfoListByType(
      productImageInfo,
      'ma',
    )
    let typeEtcImageInfoList = this.filterImageInfoListByType(
      productImageInfo,
      'et',
    )
    let orderedAndFilteredList = this.orderImageSequence(
      typeMainImageInfoList,
      typeEtcImageInfoList,
      LIMIT,
    )
    ...
    resultHTML += templating.bindingTemplate(
      productImageItemTemplate,
      lastImage,
    )
...
function templatingComments(
  ...
  const shortReviewItem = document.querySelector('#list_short_review_item')
    .innerHTML
  const noImageShortReviewItem = document.querySelector(
    '#list_short_review_item_no_img',
  ).innerHTML
  ...
  $('.prev').click(function () {
    ...
    if (imageList.children[nowImageIndex].id === 'FIRST') {
      ...
      imageList.style.transform =
        'translateX(' + nowImageIndex * itemWidth + 'px)'
    } else {
      nowImageIndex--
      $('#nowIndex').text(nowImageIndex)
      imageList.style.transitionDuration = '0.2s'
...
function clickSectionInfoTab() {
  document
    .querySelector('.section_info_tab')
