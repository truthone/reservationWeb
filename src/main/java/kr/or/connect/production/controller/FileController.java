...
public class FileController {
  ...
  public String uploadForm(
    ...
    Long productId = reservationService
      .getReservationInfo(reservationInfoId)
      .get(0)
      .getProductId();
    String description = productDetailService
      .getDisplayInfo(productId)
      .get(0)
  ...
  public String upload(
    @RequestParam(name = "reviewImg", required = false) MultipartFile file,
    ...
    Long reservationUserCommentId = reservationCommentService.addReservationComment(
      reservationUserCommentInfo
    );

    try (
      FileOutputStream fos = new FileOutputStream(
        DIRECTORY_PATH + file.getOriginalFilename()
      );
      ...
      reservationUserCommentImageInfo.setReservationUserCommentId(
        reservationUserCommentId
      );

      reservationCommentService.addReservationCommentImageInfo(
        reservationUserCommentImageInfo
      );
    } catch (Exception ex) {
