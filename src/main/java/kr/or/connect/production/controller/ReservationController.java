	...
	public String reservation(@ModelAttribute ReservationInfo reservationInfo) {
		...
		if (reservationInfoPriceList != null) {
			for(int i =0; i < reservationInfoPriceList.size(); i++) {
				...
				reservationInfoPrice.setProductPriceId(reservationInfoPriceList.get(i).getProductPriceId());
				reservationInfoPrice.setCount(reservationInfoPriceList.get(i).getCount());
